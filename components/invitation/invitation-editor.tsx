...
export function InvitationEditor({ template }: { template: InvitationTemplate }) {
  ...
  return (
    <main className="min-h-screen bg-[#ded8ce]">
      <header className="flex flex-wrap items-center justify-between gap-4 border-b border-black/10 bg-[#fffdf8] px-4 py-3 sm:px-6"><div className="flex items-center gap-4"><Link href={`/templates/${template.slug}`} className="rounded-full p-2 hover:bg-black/5" aria-label="Volver"><ArrowLeft className="size-5"/></Link><span className="font-serif text-xl tracking-[.14em]">{BRAND_NAME}</span><span className="hidden text-xs text-muted-foreground sm:inline">Editando · {template.name}</span></div><div className="flex gap-2"><button type="button" onClick={save} className="inline-flex items-center gap-2 rounded-full border border-black/15 px-4 py-2 text-xs"><Save className="size-4"/>{saved ? "Guardado" : "Guardar"}</button><Link href={`/invitation/${template.slug}`} onClick={save} className="inline-flex items-center gap-2 rounded-full bg-foreground px-4 py-2 text-xs text-background">Publicar <ExternalLink className="size-3.5"/></Link></div></header>
